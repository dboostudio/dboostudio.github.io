...
author: Dboo
categories: 독서노트
title: 클린코드 CH 1. 깨끗한 코드, CH 2. 의미 있는 이름
...
- 인터페이스 클래스와 구현 클래스
    꼭 인코딩해야한다면 인터페이스보다는 구현클래스의 이름을 인코딩하는것이 좋다.  
    ex) IShpaeFactory, ShapeFactory < ShapeFactory, ShapeFactoryImp  
    하지만, 각 구현체에 의미있는 이름을 부여하는 것이 더 좋다.  
    ex) ShapeFactory -> TriangleFactory, RectangleFactory, ...
